...
def get_work_colleagues():
    ...
    current = work[work['Status'] != 'Done']
    # Select columns and rename them to match your lowercase/snake_case requirements
    df_subset = current[['Task', 'Date', 'Status', 'Priority', 'Formula']].copy()
    df_subset.columns = ['task', 'date', 'status', 'priority', 'relative_due']
    df_subset.sort_values(by='date', ascending=False, inplace=True, ignore_index=True)
